  ...
  class _OrderDetailsBottomSheetState extends State<OrderDetailsBottomSheet> {
    ...
    bool _isOrderCompleted(dynamic order) {
    final status = (order.agentDeliveryStatus ?? '').toString().toLowerCase();
  ...
  Widget _buildAcceptRejectButtons(dynamic order) {
    ...
    // 🔥 CLOSE BOTTOM SHEET AFTER PICKUP
    if (newStage == DeliveryStage.goingToCustomer) {
      await Future.delayed(const Duration(milliseconds: 300));
      if (mounted) {
